...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td valign="top">
			<select name="material" id="material" class="sel-plano" style="width:100%;" size="15" 
				onkeypress="javascript: return KeyPress(event, this.selectedIndex);"
			>
			<?php
			$stmt = mssql_query("EXEC sp_getMateriales '$texto','C', 'M', '$bodega', 'OC'",$cnx);
			if($rst = mssql_fetch_array($stmt)){
				do{
					$arrDatos .= "'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDescripcion"]).'&&&'.trim(strtoupper($rst["strUnidad"])).'&&&'.trim($rst["dblStock"])."',";
					echo '<option value="'.trim($rst["strCodigo"]).'">['.trim($rst["strCodigo"]).'] '.trim(substr($rst["strDescripcion"], 0, 53)).' ['.htmlentities($rst["strUnidad"]).']</option>';
