...
<!--
var oculta=0;
setInterval("OcultaMenuAutomatico(oculta, 'tdInmobiliaria', 'aInmobiliaria', 'divInmobiliaria')", 3000);

function Load(){
	AjustaMenu('frmInmobiliaria', 'tdInmobiliaria', document.getElementById('alto').value, document.getElementById('ancho').value);
}
-->
</script>
<body marginheight="0" marginwidth="0" onload="javascript: Load();">
	...
	<?php 
	$stmt = mssql_query("EXEC General..sp_getMenus '$usuario', 'P'", $cnx);
	while($rst = mssql_fetch_array($stmt)){
		...
		echo '<tr height="20px">';
		echo '<td width="20px" bgcolor="#8F5F4A">&nbsp;</td>';
		echo '<td id="tdP'.$opcion.'" nowrap="nowrap">&nbsp;';
		echo '<a id="aP'.$opcion.'" href="#"';
		echo 'onclick="javascript: ';
		echo "CreaVentana('".$rst["strLink"].$parametros."', '".$rst["strMenu"]."', 98, 350, true);";
		echo "OcultaMenu('tdInmobiliaria', 'aInmobiliaria', 'divInmobiliaria', true, true);";
		echo '"';
		echo 'onmouseover="javascript: ';
		echo 'oculta=0;';
		echo "MuestraMenu('tdP$opcion', 'aP$opcion');";
		echo '"';
		echo 'onmouseout="javascript: ';
		echo 'oculta=1;';
		echo "OcultaMenu('tdP$opcion', 'aP$opcion');";
		echo '"';
		echo '>'.$rst["strMenu"].'</a>';
		echo '</td>';
		echo '</tr>';
