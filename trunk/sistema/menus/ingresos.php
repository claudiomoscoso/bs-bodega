...
<title>Opciones de Ingreso</title>
<style>
...
<!--
var oculta=0;
setInterval("OcultaMenuAutomatico(oculta, 'tdIngreso', 'aIngreso', 'divIngreso')", 3000);

function Load(){
	AjustaMenu('frmIngreso', 'tdIngreso', document.getElementById('alto').value, document.getElementById('ancho').value);
}
-->
</script>
<body marginheight="0" marginwidth="0" onload="javascript: Load();">
	...
	<?php 
	$stmt = mssql_query("EXEC General..sp_getMenus '$usuario', 'I'", $cnx);
	while($rst = mssql_fetch_array($stmt)){
		...
		echo '<tr height="20px">';
		echo '<td width="20px" bgcolor="#8F5F4A">&nbsp;</td>';
		echo '<td id="tdI'.$opcion.'" nowrap="nowrap">&nbsp;&nbsp;';
		echo '<a id="aI'.$opcion.'" href="#" ';
		echo 'onclick="javascript: ';
		echo "CreaVentana('".$rst["strLink"].$parametros."', '".$rst["strMenu"]."', 95, 350, true);";
		echo "OcultaMenu('tdIngreso', 'aIngreso', 'divIngreso', true, true);";
		echo '"';
		echo 'onmouseover="javascript: ';
		echo 'oculta=0;';
		echo "MuestraMenu('tdI$opcion', 'aI$opcion');";
		echo '"';
		echo 'onmouseout="javascript: ';
		echo 'oculta=1;';
		echo "OcultaMenu('tdI$opcion', 'aI$opcion');";
		echo '"';
		echo '>'.$rst["strMenu"].'</a>';
		echo '</td>';
		echo '</tr>';
