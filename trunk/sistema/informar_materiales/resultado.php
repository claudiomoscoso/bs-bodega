...
if($_GET["movil"]=='none' || $_GET["movil"] == '') $movil = 'NULL'; else $movil="'".$_GET["movil"]."'";
$numot=$_GET["numot"] != '' ? "'".$_GET["numot"]."'" : 'NULL';
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" />
<title>Informae Materiales</title>
</head>
...
<?php
$stmt = mssql_query("EXEC Orden..sp_getOrdenTrabajo 2, '', '$contrato', $movil, $numot, '$perfil'", $cnx);
if($rst=mssql_fetch_array($stmt)){
	...
	<tr bgcolor="<?php echo ($cont % 2)==0 ? '#FFFFFF' : '#EBF3FE';?>">
		...
		<td width="15%" align="left">
			<input type="hidden" name="hdnContrato<?php echo $cont;?>" id="hdnContrato<?php echo $cont;?>" value="&nbsp;<?php echo htmlentities(trim($rst["strDescContrato"]));?>" />
			<input name="txtContrato<?php echo $cont;?>" id="txtContrato<?php echo $cont;?>" class="txt-sborde" style="width:99%; background-color:<?php echo ($cont % 2)==0 ? '#FFFFFF' : '#EBF3FE';?>" readonly="true" value="&nbsp;<?php echo htmlentities(trim($rst["strDescContrato"]));?>" 
				onmouseover="javascript:
					clearInterval(Intervalo); 
					Intervalo=setInterval('MueveTexto(\''+this.id+'\', \'hdnContrato<?php echo $cont;?>\')', 250);
				"
				onmouseout="javascript:
					DetieneTexto(Intervalo, this.id, 'hdnContrato<?php echo $cont;?>');
				"
			/>
		</td>
		<td width="15%" align="left">
