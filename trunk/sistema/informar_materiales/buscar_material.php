...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="3" valign="top">
			...
			<?php
				if($tipo == 1)
					$sql = "EXEC Bodega..sp_getMateriales '$texto', 'C', 'T', '$movil', 'INF'";
				else
					$sql = "EXEC Bodega..sp_getMateriales '$texto', 'C', 'T', '$contrato', 'INF2'";
				
				$stmt = mssql_query($sql, $cnx);
				if($rst=mssql_fetch_array($stmt)){
					do{
						$arrDatos.="'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDescripcion"]).'&&&'.trim($rst["strUnidad"]).'&&&'.trim($rst["dblCantidad"])."',";
						echo '<option value="'.$rst["strCodigo"].'">['.trim($rst["strCodigo"]).'] '.($rst["CODROSS"] != '' ? '*' : '').' '.$rst["strDescripcion"].' ['.trim($rst["strUnidad"]).']</option>';
		...
		switch(idCtrl){
			case 'material':
				...
				if(document.getElementById('material').value != 'none' && document.getElementById('material').value!=''){
					...
					parent.document.getElementById('hdnUnidad').value = arrPaso[2];
					parent.document.getElementById('hdnCantidad').value = arrPaso[3];
