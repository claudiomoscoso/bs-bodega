...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="3" valign="top">
			<select name="material" id="material" class="sel-plano" style="width:100%;" size="14" 
				onkeypress="javascript: return Saltar(event, this.selectedIndex, this.id);"
			>
			<?php
				$stmt = mssql_query("EXEC Bodega..sp_getMateriales '$texto', 'C', NULL, NULL, 'DC'",$cnx);
				if($rst=mssql_fetch_array($stmt)){
					do{
						$arrDatos.="'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDescripcion"])."',";
						echo '<option value="'.trim($rst["strCodigo"]).'">'.'['.trim($rst["strCodigo"]).'] '.$rst["strDescripcion"].'</option>';
		...
		switch(idCtrl){
			case 'material':
				...
				if(document.getElementById('material').value != 'none' && document.getElementById('material').value!=''){
					...
					parent.document.getElementById('hdnMaterial').value = arrPaso[0];
					parent.document.getElementById('<?php echo $ctrl;?>').value = '[' + arrPaso[0] + '] ' + Reemplazar(arrPaso[1]);
