...
$stmt = mssql_query("EXEC Bodega..sp_getDevolucionesPendientes 6, NULL, '$bodega', '$cargo'", $cnx);
if($rst = mssql_fetch_array($stmt)){
	do{
		$cont++;
		echo '<tr bgcolor="'.($cont % 2 == 0 ? '#EBF1FF' : '#FFFFFF').'">';
		echo '<td width="3%" align="center">'.$cont.'</td>';
		echo '<td width="10%" align="center">'.$rst["dblNumero"].'</td>';
		echo '<td width="10%" align="center">'.$rst["strCodigo"].'</td>';
		echo '<td width="25%" align="left">&nbsp;'.htmlentities($rst["strDescripcion"]).'</td>';
		echo '<td width="20%" align="left">&nbsp;'.htmlentities($rst["strNombre"]).'</td>';
		echo '<td width="10%" align="right">'.$rst["dblRetiro"].'&nbsp;</td>';
		echo '<td width="10%" align="right">'.$rst["dblDevuelto"].'&nbsp;</td>';
