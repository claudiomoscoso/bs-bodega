...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="3" valign="top">
			...
			>
          	<?php
			$stmt = mssql_query("EXEC General..sp_getCargos 10, NULL, '$texto', '$usuario', 'C'", $cnx);
			if($rst = mssql_fetch_array($stmt)){
				do{
					$arrDatos .= "'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDetalle"])."',";
					echo '<option value="'.$rst["strCodigo"].'">['.trim($rst["strCodigo"]).'] '.$rst["strDetalle"].'</option>';
		...
		switch(idCtrl){
			case 'lista':
				...
				if(document.getElementById('lista').value != 'none' && document.getElementById('lista').value != ''){
					...
					parent.document.getElementById('hdnObra').value = arrPaso[0];
					parent.document.getElementById('txtObra').value = ' ' + Reemplazar(arrPaso[1]);
