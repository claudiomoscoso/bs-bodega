...
<table border="0" width="100%" cellpadding="0" cellspacing="0">
	<tr>
		...
		<th width="12%" align="left">&nbsp;Equipo</th>
		<th width="14%" align="left">&nbsp;Ult.Ubicaci&oacute;n</th>
		<th width="10%">Estado</th>
		<th width="7%">Kms.Inicial</th>
		<th width="7%">Kms.Final</th>
		<th width="7%">Kms.Total</th>
		<th width="6%">N&deg;Report</th>
		<th width="5%">Litros</th>
		<th width="15%" align="left">&nbsp;Operador</th>
		<th width="5%">O.T.</th>
		<th width="2%">&nbsp;</th>
	</tr>
</table>
...
if($rst = mssql_fetch_array($stmt)){
	do{
		...
		if($rst["dblFallas"] > 0){
			echo '<a href="#" title="Abre cuadro de dialogo"';
			echo ' onclick="javascript: ';
			echo 'parent.Deshabilita(true); ';
			echo "AbreDialogo('divFallas', 'frmFallas', 'reportes_fallas.php?ccosto=".$rst["strCCosto"]."', true)";
			echo '"';
			echo 'onmouseover="javascript: window.status=\'Abre cuadro de dialogo.\'; return true;"';
			echo '><img id="imgFalla'.$cont.'" border="0" align="center" src="../images/urgente.gif"></a>';
		}else
			echo '&nbsp;';
		echo '</td>';
		echo '<td width="6%" align="center">'.$rst["strCCosto"].'</td>';
		...
		echo '/>';
		echo '</td>';
		echo '<td width="14%" align="left">';
		echo '<input type="hidden" name="hdnUbicacion'.$cont.'" id="hdnUbicacion'.$cont.'" value="'.htmlentities($rst["strUbicacion"]).'" />';
		echo '<input name="txtUbicacion'.$cont.'" id="txtUbicacion'.$cont.'" class="txt-sborde" style="width:100%; background-color:'.($cont % 2 == 0 ? '#FFFFFF' : '#EBF3FE').'" readonly="true" value="'.htmlentities($rst["strUbicacion"]).'" ';
		echo 'onmouseover="javascript: ';
		echo 'clearInterval(Intervalo); ';
		echo "Intervalo = setInterval('MueveTexto(\''+this.id+'\', \'hdnUbicacion$cont\')', 250);";
		echo '"';
		echo 'onmouseout="javascript: ';
		echo "DetieneTexto(Intervalo, this.id, 'hdnUbicacion$cont');";
		echo '"';
		echo '/>';
		echo '</td>';
		echo '<td width="10%">';
		...
		echo '/>';
		echo '</td>';
		echo '<td width="7%" align="center">'.number_format($rst["dblOdometroInicial"], 2, ',', '.').'</td>';
		echo '<td width="7%" align="center">'.number_format($rst["dblOdometroFinal"], 2, ',', '.').'</td>';
		echo '<td width="7%" align="center">'.number_format($rst["dblOdometroFinal"] - $rst["dblOdometroInicial"], 2, ',', '.').'</td>';
		echo '<td width="6%" align="center">'.$rst["dblNumero"].'</td>';
		echo '<td width="5%" align="center">'.number_format($rst["dblCombustible"], 2, ',', '.').'</td>';
		...
		echo '/>';
		echo '</td>';
		echo '<td width="5%" align="center">'.$rst["strOTrabajo"].'</td>';
		echo '</tr>';
