...
if(($tipodoc==0 || $tipodoc==1 || $tipodoc==2 || $tipodoc==6) && $tbusqueda==0)
	$sql= "EXEC Bodega..sp_getOrdenCompra 'NUM', $observacion, '%', '$bodega'";
elseif($tipodoc==6 && $tbusqueda==1)
...
function MuestraDocumento(numero){
	...
	AbreDialogo('divDocumento', 'frmDocumento', url+'?bodega=<?php echo $bodega;?>&numero='+numero, true);
}
-->
</script>
...
$stmt = mssql_query("EXEC Bodega..sp_getBuscaDocumento '$bodega', $tipodoc, $fecha, $material, $proveedor, $observacion, $numero, $tbusqueda", $cnx);
if($rst=mssql_fetch_array($stmt)){
	do{
		...
		echo '<td width="10%" align="center">
			     <a href="#" '.($rst["strEstado"]==5 ? 'style="color:#FF0000"' : '').' title="Ver documento Nº '.$rst["dblNum"].'..."
				    onclick="javascript: 
