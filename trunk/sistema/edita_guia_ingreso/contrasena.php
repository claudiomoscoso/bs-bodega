...
$numoc=$_GET["numoc"] !='' ? $_GET["numoc"] : $_POST["hdnOCompra"];
...
if($clave!=''){ 
	...
	if($rst=mssql_fetch_array($stmt)){
		...
		if($obs!='') mssql_query("EXEC Bodega..sp_GrabaObservacion $numero,'OC','".Reemplaza($obs)."','$usuario'", $cnx);
		$estado=0;
	}else
		$estado=1;
...
<input type="hidden" name="hdnOCompra" id="hdnOCompra" value="<?php echo $numoc;?>" />
