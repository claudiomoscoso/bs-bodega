...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td valign="top">
			<select name="material" id="material" class="sel-plano" style="width:100%;" size="10" onkeypress="javascript: return KeyPress(event, this.selectedIndex);">
			<?php
			$stmt = mssql_query("EXEC Bodega..sp_getMateriales '$texto', 'C', NULL, '$bodega', 'GCAR'",$cnx);
			if($rst=mssql_fetch_array($stmt)){
				do{
					$arrDatos.="'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDescripcion"]).'&&&'.trim($rst["strUnidad"]).'&&&'.trim($rst["dblStock"])."',";
					echo '<option value="'.trim($rst["strCodigo"]).'">'.trim($rst["strDescripcion"]).' ['.trim($rst["strUnidad"]).']</option>';
