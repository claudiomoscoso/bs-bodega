...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td valign="top">
			<select name="lista" id="lista" class="sel-plano" style="width:100%;" size="16" 
			...
			if($rst = mssql_fetch_array($stmt)){
				do{
					$arrDatos .= "'".trim($rst["strCCosto"]).'&&&'.$rst["dblPrecio"]."',";
					echo '<option value="'.$rst["strCCosto"].'">['.trim($rst["strCCosto"]).'] '.$rst["strDescripcion"].(trim($rst["strPatente"]) != '' ? ' ['.trim($rst["strPatente"]).']' : '').'</option>';
...
function KeyPress(evento, ind){
	...
	if(tecla == 27){
		...
		if(document.getElementById('lista').value != 'none' && document.getElementById('lista').value != ''){
			...
			parent.document.getElementById('txtValor').value = arrPaso[1];
			...
			parent.document.getElementById('txtValor').value = 0;
			parent.document.getElementById('cmbDPago').focus();
			parent.document.getElementById('cmbDPago').select();
		}
	}
}
