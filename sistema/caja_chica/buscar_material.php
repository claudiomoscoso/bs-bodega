...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="3" valign="top">
			...
			>
			<?php
				$stmt = mssql_query("EXEC Bodega..sp_getMateriales '$texto', 'C', NULL, '$bodega', 'DC'", $cnx);
				if($rst=mssql_fetch_array($stmt)){
					do{
						$arrDatos.="'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDescripcion"]).'&&&'.trim($rst["strUnidad"])."',";
						echo '<option value="'.$rst["strCodigo"].'">'.htmlentities('['.trim($rst["strCodigo"]).'] '.trim($rst["strDescripcion"])).' ('.trim($rst["strUnidad"]).')</option>';
		...
		switch(idCtrl){
			case 'material':
				...
				if(document.getElementById('material').value != 'none' && document.getElementById('material').value!=''){
					...
					parent.document.getElementById('hdnUnidad').value = Reemplazar(arrPaso[2]);
