...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td valign="top">
			...
			>
			<?php
			$stmt = mssql_query("EXEC Bodega..sp_ListarMatCStockXBod '$texto', '$bodega'",$cnx);
			if($rst=mssql_fetch_array($stmt)){
				do{
					$arrDatos.="'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDetalleMaterial"]).'&&&'.trim($rst["strUnidad"]).'&&&'.trim($rst["dblStock"])."',";
					echo '<option value="'.trim($rst["strCodigo"]).'">['.trim($rst["strCodigo"]).'] '.$rst["strDetalleMaterial"].' ('.trim($rst["dblStock"]).')</option>';
