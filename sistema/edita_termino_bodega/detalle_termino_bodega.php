...
<?php
$stmt = mssql_query("EXEC Bodega..sp_getTMPTerminoBodega 3, '$usuario'", $cnx);
while($rst=mssql_fetch_array($stmt)){
	...
	<tr bgcolor="<?php echo ($cont % 2 == 0 ? '#EBF1FF' : '#FFFFFF');?>">
		...
		<td width="10%" align="center"><?php echo number_format($rst["dblStock"], 2, '.', '');?></td>
		<td width="10%" >
			<input type="hidden" name="hdnCantidad<?php echo $cont;?>" id="hdnCantidad<?php echo $cont;?>" value="<?php echo number_format($rst["dblDevolucion"], 2, '.', '');?>" />
			<input name="txtCantidad<?php echo $cont;?>" id="txtCantidad<?php echo $cont;?>" class="txt-plano" style="width:99%;text-align:right" disabled="disabled" value="<?php echo number_format($rst["dblDevolucion"], 2, '.', '');?>" 
