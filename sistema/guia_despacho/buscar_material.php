...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td valign="top">
			...
			<?php
			$stmt = mssql_query("EXEC Bodega..sp_getMateriales '$texto', 'C', 'M', '$bodega', 'GD', $tcargo", $cnx);
			if($rst = mssql_fetch_array($stmt)){
				do{
					$arrDatos .= "'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDescripcion"]).'&&&'.trim($rst["strUnidad"]).'&&&'.trim($rst["dblStock"])."',";
					echo '<option value="'.trim($rst["strCodigo"]).'">'.trim($rst["strDescripcion"]).' ['.trim($rst["strUnidad"]).'] ('.number_format($rst["dblStock"], 2, ',', '.').')</option>';
