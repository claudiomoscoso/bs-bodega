...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="3" valign="top">
			...
			>
			<?php
				$stmt = mssql_query("EXEC Bodega..sp_getMateriales '$texto', 'C', 'M', '$bodega', 'GCAR'", $cnx);
				if($rst=mssql_fetch_array($stmt)){
					do{
						$arrDatos.="'".trim($rst["strCodigo"]).'&&&'.trim($rst["strDescripcion"])."',";
						echo '<option value="'.trim($rst["strCodigo"]).'">['.trim($rst["strCodigo"]).'] '.htmlentities(trim($rst["strDescripcion"])).'</option>';
