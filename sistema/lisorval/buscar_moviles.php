...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="3" valign="top"><select name="lista" id="lista" class="sel-plano" style="width:100%;" size="14" 
          ...
          <?php
			...
			if($rst=mssql_fetch_array($stmt)){
				do{
					$arrDatos.="'".trim($rst["strMovil"]).'&&&'.trim($rst["strNombre"])."',";
					echo '<option value="'.$rst["strMovil"].'">['.trim($rst["strMovil"]).'] '.$rst["strNombre"].'</option>';
...
function KeyPress(evento, ind, idCtrl){
	...
	if(tecla == 27){
		...
		parent.CierraDialogo('divMovil','frmMovil');
		...
		switch(idCtrl){
			case 'lista':
				...
				if(document.getElementById('lista').value != 'none' && document.getElementById('lista').value!=''){
					...
					parent.document.getElementById('hdnMovil').value = arrPaso[0];
					parent.document.getElementById('txtMovil').value = ' ' + Reemplazar(arrPaso[1]);
					parent.document.getElementById('<?php echo $foco;?>').focus();
				}else{
					parent.document.getElementById('hdnMovil').value = '';
					parent.document.getElementById('txtMovil').value = '';
					parent.document.getElementById('<?php echo $ctrl;?>').focus();
