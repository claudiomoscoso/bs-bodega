...
$sql = "SELECT dblNumero, CONVERT(VARCHAR, dtmFecha, 103) AS dtmFecha, CASE blnReEnvio WHEN 0 THEN 'Env&iacute;o' ELSE 'Reenv&iacute;o' END AS strEnvio FROM Orden..Envio WHERE strContrato='$contrato'";
if($texto != '') $sql.=" AND dblNumero LIKE '$texto%'";
$sql.=" ORDER BY dblNumero DESC";
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" />
<title>&Uacute;ltimos Env&iacute;os</title>
</head>
	...
	<tr>
		<td colspan="3" valign="top">
			...
			if($rst = mssql_fetch_array($stmt)){
				do{
					$arrDatos.="'".trim($rst["dblNumero"])."',";
					echo '<option value="'.$rst["dblNumero"].'">'.trim($rst["dblNumero"]).' | '.$rst["dtmFecha"].' | '.$rst["strEnvio"].'</option>';
...
function KeyPress(evento, ind, idCtrl){
	...
	if(tecla == 27){
		...
		parent.CierraDialogo('divNumero','frmNumero');
		...
		switch(idCtrl){
			case 'lista':
				...
				if(document.getElementById('lista').value != 'none' && document.getElementById('lista').value!=''){
					...
					parent.document.getElementById('txtNEnvio').value = arrPaso[0];
