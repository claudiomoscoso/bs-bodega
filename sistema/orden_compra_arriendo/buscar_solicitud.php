...
<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td valign="top">
			...
			>
			<?php
				$stmt = mssql_query("EXEC Operaciones..sp_getSolicitudes 'SP', NULL, '$cargo'",$cnx);
				if($rst=mssql_fetch_array($stmt)){
					do{
						$arrDatos.="'".trim($rst["dblNumero"])."',";
						echo '<option value="'.$rst["dblNumero"].'">'.$rst["dblNumero"].' - '.$rst["dtmFecha"].' - '.substr($rst["strDescripcion"], 0, 25).'</option>';
...
function Saltar(evento, ind){
	...
	if(tecla==27){
		...
		if(document.getElementById('lista').value != 'none' && document.getElementById('lista').value!=''){
			...
			parent.document.getElementById('txtSolicitud').value = arrPaso[0];
