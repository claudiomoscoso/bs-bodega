...
<!--
var oculta=0;
setInterval("OcultaMenuAutomatico(oculta, 'tdConsulta', 'aConsulta', 'divConsulta')", 3000);

function Load(){
	AjustaMenu('frmConsulta', 'tdConsulta', document.getElementById('alto').value, document.getElementById('ancho').value);
}
-->
</script>
<body marginheight="0" marginwidth="0" onload="javascript: Load();">
	...
	<?php 
	$stmt = mssql_query("EXEC General..sp_getMenus '$usuario', 'C'", $cnx);
	while($rst = mssql_fetch_array($stmt)){
		...
		echo '<tr height="20px">';
		echo '<td width="20px" bgcolor="#8F5F4A">&nbsp;</td>';
		echo '<td id="tdC'.$opcion.'" nowrap="nowrap">&nbsp;';
		echo '<a id="aC'.$opcion.'" href="#"';
		echo 'onclick="javascript: ';
		echo "CreaVentana('".$rst["strLink"].$parametros."', '".$rst["strMenu"]."', 98, 350, true);";
		echo "OcultaMenu('tdConsulta', 'aConsulta', 'divConsulta', true, true);";
		echo '"';
		echo 'onmouseover="javascript: ';
		echo 'oculta=0;';
		echo "MuestraMenu('tdC$opcion', 'aC$opcion');";
		echo '"';
		echo 'onmouseout="javascript: ';
		echo 'oculta=1;';
		echo "OcultaMenu('tdC$opcion', 'aC$opcion');";
		echo '"';
		echo '>'.$rst["strMenu"].'</a>';
		echo '</td>';
		echo '</tr>';
