...
<!--
var oculta=0;
setInterval("OcultaMenuAutomatico(oculta, 'tdAutorizar', 'aAutorizar', 'divAutorizar')", 3000);

function Load(){
	AjustaMenu('frmAutorizar', 'tdAutorizar', document.getElementById('alto').value, document.getElementById('ancho').value);
}
-->
</script>
<body marginheight="0" marginwidth="0" onload="javascript: Load();">
	...
	<?php 
	$stmt = mssql_query("EXEC General..sp_getMenus '$usuario', 'A'", $cnx);
	while($rst = mssql_fetch_array($stmt)){
		...
		echo '<tr height="20px">';
		echo '<td width="20px" bgcolor="#8F5F4A">&nbsp;</td>';
		echo '<td id="tdA'.$opcion.'" nowrap="nowrap">&nbsp;';
		echo '<a id="aA'.$opcion.'" href="#"';
		echo 'onclick="javascript: ';
		echo "CreaVentana('".$rst["strLink"].$parametros."', '".$rst["strMenu"]."', 98, 350, true);";
		echo "OcultaMenu('tdAutorizar', 'aAutorizar', 'divAutorizar', true, true);";
		echo '"';
		echo 'onmouseover="javascript: ';
		echo 'oculta=0;';
		echo "MuestraMenu('tdA$opcion', 'aA$opcion');";
		echo '"';
		echo 'onmouseout="javascript: ';
		echo 'oculta=1;';
		echo "OcultaMenu('tdA$opcion', 'aA$opcion');";
		echo '"';
		echo '>'.$rst["strMenu"].'</a>';
		echo '</td>';
		echo '</tr>';
